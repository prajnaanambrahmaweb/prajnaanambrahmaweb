jQuery(function($) {
  ...
  var uniteController = {
    init: function(opts) {
      ...
      $(window).scroll(function(){
        var scaler = 1 - $(window).scrollTop() / ($('.banner-wrap').height());
          $('body.header-scroll-animate-on:not(.wsite-editor) .banner').css("opacity", 1 - $(window).scrollTop() / ($('.banner-wrap').height() / 1.5));
          $('body.header-scroll-animate-on:not(.wsite-editor) .banner').css("top", - 200 * $(window).scrollTop() / $('.banner-wrap').height());
          $('body.header-scroll-animate-on:not(.wsite-editor) .banner').css({'-webkit-transform' : 'scale(' + scaler + ')', '-moz-transform' : 'scale(' + scaler + ')', '-ms-transform' : 'scale(' + scaler + ')', 'transform' : 'scale(' + scaler + ')'});
      });
    ...
    _attachEvents: function() {
        ...
        // Move cart
        $.fn.intervalLoop('.unite-header #wsite-menus', base._moveFlyout, 300, 8);
